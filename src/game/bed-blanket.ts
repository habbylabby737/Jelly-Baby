...
export class BedBlanket {
  ...
  reset(){
    ...
    for(let z=0;z<this.rows;z++)for(let x=0;x<this.columns;x++){
      ...
      this.positions[j+1]=this.base(x);
    }
  }
  ...
  step(h:number,body:SoftBody,occupied:boolean){
    for(let z=0;z<this.rows;z++)for(let x=0;x<this.columns;x++)this.contact[z*this.columns+x]=this.base(x);
    if(occupied)this.bodyContact.sample(body,this.contact,this.columns,this.rows,this.dx,this.dz);
    for(let z=0;z<this.rows;z++)for(let x=0;x<this.columns;x++){
      const i=z*this.columns+x,y=this.positions[i*3+1];let sum=0,count=0;
      for(const [ox,oz] of NEIGHBORS){
        const nx=x+ox,nz=z+oz;if(nx<0||nx>=this.columns||nz<0||nz>=this.rows)continue;
        sum+=this.positions[(nz*this.columns+nx)*3+1]-y;count++;
      }
